...
namespace Pu
{
	...
	public:
		...
		*/
		...
		/* Gets the size (in bytes) of the vertices generated by the patch list plane or the triangle plane. */
		...
		/* Gets the size (in bytes) of the vertices generated by the volume sphere. */
		_Check_return_ static uint32 GetVolumeSphereVertexSize(_In_ uint16 divisions);
		...
		/* Gets the required buffer size (in bytes) for a volume sphere. */
		_Check_return_ static size_t GetVolumeSphereBufferSize(_In_ uint16 divisions);
		...
		_Check_return_ static Mesh Plane(_In_ Buffer &src, _In_opt_ bool mapMemory =  true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a triangle list plane and returns a mesh that can view that plane. */
		_Check_return_ static Mesh LodPlane(_In_ Buffer &src, _In_ uint16 divisions, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a patch list plane and returns a mesh that can view that plane. */
		_Check_return_ static Mesh PatchPlane(_In_ Buffer &src, _In_ uint16 divisions, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a basic box and returns a mesh that can view that box. */
		_Check_return_ static Mesh Box(_In_ Buffer &src, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a spherified cube and returns a mesh that can view that sphere. */
		_Check_return_ static Mesh Sphere(_In_ Buffer &src, _In_ uint16 divisions, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a position only spherified cube and returns a mesh that can view that volume sphere. */
		_Check_return_ static Mesh VolumeSphere(_In_ Buffer &src, _In_ uint16 divisions, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a (UV) hemisphere and returns a mesh that can view that hemisphere. */
		_Check_return_ static Mesh Dome(_In_ Buffer &src, _In_ uint16 divisions, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a torus and returns a mesh that can view that torus. */
		_Check_return_ static Mesh Torus(_In_ Buffer &src, _In_ uint16 divisions, _In_ float ratio, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a cylinder and returns a mesh that can view that cylinder. */
		_Check_return_ static Mesh Cylinder(_In_ Buffer &src, _In_ uint16 divisions, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
		/* Populates the source buffer with a cone and returns a mesh taht can view that cone. */
		_Check_return_ static Mesh Cone(_In_ Buffer &src, _In_ uint16 divisions, _In_opt_ bool mapMemory = true, _In_ DeviceSize offset = 0);
