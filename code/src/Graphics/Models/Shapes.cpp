...
void Plutonium::ShapeCreator::MakePlane(Mesh * mesh)
{
	mesh->SetBufferSize(6);

	VertexFormat *vrtx = mesh->GetVertexAt(0);
	vrtx->Position = Vector3(-1.0f, 1.0f, 0.0f);
...
void Plutonium::ShapeCreator::MakeBox(Mesh * mesh)
{
	mesh->SetBufferSize(36);

	VertexFormat *vrtx = mesh->GetVertexAt(0);
	vrtx->Position = Vector3(-1.0f, 1.0f, -1.0f);
	vrtx->Normal = Vector3::Forward;
	vrtx->Texture = Vector2::One;
...
void Plutonium::ShapeCreator::MakeSphere(Mesh * mesh, size_t meridians, size_t parallels)
{
	...
	for (size_t i = 0; i < parallels; i++)
	{
		for (size_t j = 0; j < meridians; j++)
		{
			...
			Vector3 pos = Vector3(c * cosf(theta), sinf(phi), c * sinf(theta));
